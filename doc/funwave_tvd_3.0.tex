...
(\bf{ Version 3.0 Revision July 2016})
}
...
\vspace*{0.5cm}
 ...
 % The work of benchmark tests 
%was sponsored by the NTHMP project. 
...
The documentation provides derivations of the conservation form of theoretical equations, re-arrangement of pressure gradient term in order to obtain a numerically well-balanced form, detailed numerical schemes, users' manual and examples.  Users can also refer to Shi et al. (2012a) for the Cartesian version and Kirby et al. (2012) for the spherical version of the model.  
 ...
 The examples in the previous report (Shi et al., 2011)  have been updated using the present version (Version 3.0). The tsunami benchmark tests in spherical coordinates are reported separately in Tehranirad et al. (2013). 
 
 Special thanks to  Dr. Seung-Nam Seo and Young-Kwang Choi from Korea Institute of Ocean Science and Technology (KIOST) who conducted line-by-line checking of the code and provided some important revisions on Version 3.0.  

\end{abstract}

\vspace*{0cm}

{\bf Source code download:} 
...
In addition, we include a documentation of the spherical Boussinesq model used for Tsunami wave simulations.  The spherical Boussinesq model was based on Kirby et al. (2004, 2012) and implemented in the same model framework. A detailed documentation for the spherical code will be reported separately. 
...
\be 
\psi_y =  \eta_t (V_1^{\prime}-V_4) + H \left( u V_{4,x} + v V_{4,y} + U_4 v_x +V_4 v_y - U_1^{\prime \prime} -  V_2 -  V_3 \right) 
\ee
...
a high-order MUSCL-TVD scheme  is implemented in the present model. The high-order MUSCL-TVD scheme can be written in a compact form including different orders of accuracy  from the second- to the fourth-order, according to Erduran et al. (2005) who modified Yamamoto et al.'s (1998)  fourth-order approach. 
...
The friction-type and viscous type sponge layers directly use the friction terms and diffusion terms existing in the model. The damping coefficients are just the friction coefficient $C_d$ and diffusion coefficient $\nu$ for the friction-type sponge and the viscous type sponge respectively. $C_d$ and $\nu$ used in the sponge layers are generally larger than the regular values used for physics.  Both coefficients are smoothly ramped in space at the sponge layer boundaries. 

Assuming that there is only one sponge layer on the left end of the domain. The friction coefficient for the friction-type sponge layer can be written as

\be
C_d = C_{max} \left (1-  \mbox{tanh} \frac{10 (i-1)}{I_{\mbox{width}}-1} \right)
\ee
where $C_{max}$ is the maximum value of $C_d$ used in the sponger layer. $i$ and $I_{\mbox{width}}$ represent point number and the layer width in points. Similar expressions can be obtained for sponge layers on three other ends of the domain as well as  the viscous sponge layers. 
...
may be found in Bouws {\em et al.} (1985).  In the model,  $\alpha = 8
\times 10^{-3}$ and 
$$
...
For spectral wave simulations, both significant wave height and root-mean-square wave height are calculated using zero-crossing at each grid point. Significant wave height is evaluated using Goda's (2000) formula
\be
...
The present version of FUNWAVE-TVD includes a number of options including (1) choice of serial or parallel code (2) Cartesian or spherical coordinate (Tsunami propagation mode), (3) samples, (4) one-way nesting mode,  (5) wave breaking index and aging (bubble and foam mode), and (6) different bottom friction formulas.
...
Note that, ``ITERATION" is not used any more. The Range-Kutta  time stepping with combined CLF criteria  is found to be more efficient than the iteration method. 
...
\noindent
The executable file `funwave' will be generated and  copied from /src to /work/. If you do not find 'funwave' in your /work/, go to /src/ to find the newest executable file and copy it to your /work/ directory.  Note: use `make clean' after modifying Makefile.  
...
\subsection{Subroutine and function descriptions}

NOTE: The list below is not complete.  

\begin{description}

\item {\bf ALLOCATE\_VARIABLES:}  allocate variables. It is called by MAIN.

\item {\bf BREAKING:}   print  breaking index and breaking age calculated based on  Kennedy et al. (2000).  This option is defined in input.txt. 

\item {\bf BOUNDARY\_CONDITION:}  provide boundary conditions at four side boudaries. It is called by FLUXES.

\item {\bf CAL\_DISPERSION:}  calculate dispersion terms. The first and second derivatives with respect to $x, y$ are also calculated in this subroutine.  In addition, the dispersion values in ghost cells are updated in this subroutine.  It is called by the main program. 

\item {\bf CALCULATE\_Cm\_Sm:}  calculate $C_m$ and $S_m$ used in Wei and Kirby's internal wave maker for irregular waves (TMA). Detailed formulation can be found in Shi et al. (2003). The subroutine is called by INITIALIZATION. 

\item {\bf CALCULATE\_MEAN:} calculate mean u v required by the smagorinsky mixing. Mean $\eta$ is also calculated.
Called by  MAIN. 

\item {\bf CALCULATE\_SPONGE:}  setup sponge layer. It is called by INITIALIZATION. 

\item {\bf CONSTRUCTION:}  second- and third-order interface construction. It is called by  FLUXES. It calls CONSTRUCT\_X and CONSTRUCT\_Y.

\item {\bf CONSTRUCTION\_HO:}  high order interface construction. It is called by  FLUXES. It calls CONSTRUCT\_HO\_X and CONSTRUCT\_HO\_Y.

\item {\bf CONSTRUCT\_HO\_X:}  high order interface construction of specific variables in $x$ direction. It is called by CONSTRUCTION\_HO.  

\item {\bf CONSTRUCT\_HO\_Y:}  high order interface construction of specific variables in $y$ direction. It is called by CONSTRUCTION\_HO.  

\item {\bf CONSTRUCT\_X:}  construct variables in $x$ direction. It is called by CONSTRUCTION.

\item {\bf CONSTRUCT\_Y:}  construct variables in $y$ direction. It is called by CONSTRUCTION.

\item {\bf CORRECTOR:}  corrector time scheme. It was used for a comparison between Predictor-corrector scheme and Runge-Kutta. It is not suggested using it in the present program.  

\item {\bf DelxFun:}  calculate derivatives with respect to $x$. It is called by DelxFun.  

\item {\bf DelxyFun:}  calculate derivatives with respect to $x$ and $y$.  It is called by FLUXES when the lower order construction is applied. It calls DelxFun and DelyFun.  

\item {\bf DelyFun:}  calculate derivatives with respect to $y$. It is called by DelxyFun.  

\item {\bf DERIVATIVE\_X:}  calculate first-derivative respect to $x$ in second order. The subroutine is called by CAL\_DISPERSION.

\item {\bf DERIVATIVE\_X\_High:}  calculate first-derivative respect to $x$ in higher-order. The subroutine is called by CAL\_DISPERSION.

\item {\bf DERIVATIVE\_XX:}  calculate second-derivative respect to $x$. The subroutine is called by CAL\_DISPERSION.

\item {\bf DERIVATIVE\_XY:}  calculate cross-derivative. The subroutine is called by CAL\_DISPERSION.

\item {\bf DERIVATIVE\_Y:}  calculate first-derivative respect to $y$ in second order. The subroutine is called by CAL\_DISPERSION.

\item {\bf DERIVATIVE\_Y\_High:}  calculate first-derivative respect to $y$ in higher-order. The subroutine is called by CAL\_DISPERSION.

\item {\bf DERIVATIVE\_YY:}  calculate second-derivative respect to $y$. The subroutine is called by CAL\_DISPERSION.

\item {\bf ESTIMATE\_DT:}  evaluate dt based on CFL criteria. It is called by MAIN. 

\item {\bf ESTIMATE\_HUV:}  Runge-Kutta solver. It is called by MAIN. It calls GET\_Eta\_U\_V\_HU\_HV. 

\item {\bf EXCHANGE:}  update $\eta, u, v, Hu, Hv$ and MASK  in the ghost cells. It is called by MAIN. It calls PHI\_COLL.  

\item {\bf EXCHANGE\_DISPERSION:}  update dispersion variables in the ghost cells. It is called by MAIN. It calls PHI\_COLL. 

\item {\bf FLUX\_AT\_INTERFACE:}  calculate numerical fluxes at four cell interfaces using the averaging method. It is called by FLUXES.

\item {\bf FLUX\_AT\_INTERFACE\_HLLC:}  calculate numerical fluxes at four cell interfaces using HLLC scheme. It is called by FLUXES. It calls HLLC.

\item {\bf FLUXES:}  calculate numerical fluxes. It is called by MAIN. It calls CONSTRUCTION lower order or CONSTRUCTION\_HO for higher order, WAVE\_SPEED, DelxyFun (for lower order), FLUX\_AT\_INTERFACE\_HLLC for HLL scheme or FLUX\_AT\_INTERFACE for averaging scheme, and BOUNDARY\_CONDITION. 

\item {\bf GET\_Eta\_U\_V\_HU\_HV:}  calculate $\eta, u,v,Hu$ and $Hv$. It is called by ESTIMATE\_HUV. The tridiagonal solver is used to get $u$ and $v$. Froude number cap is applied in this subroutine. It calls TRIG (wall boundary) or TRIG\_PERIODIC (periodic boundary).  In the parallel mode, it calls TRIDx and TRIDy. 

\item {\bf GetFile:} read data in the global dimension and distribute to all processors in the parallel mode. Use flag '-DPARALLEL' in Makefile to include this subroutine. 

\item {\bf GatherVariable:}  gather variables from all processors to processor 0  in the parallel mode but with serial tridiagonal solver. Use flag `-DTRID\_NO\_PARALLEL' to include this subroutine. It is called by TRIDxyNoParallel. 

\item {\bf HLLC:}  HLLC scheme. It is called by FLUX\_AT\_INTERFACE\_HLLC. 

\item {\bf INDEX:}  indexing for ghost cells and MPI subdomains.  It is called by MAIN. 

\item {\bf INITIALIZATION:}  initialization subroutine. It is called MAIN. It may call WK\_WAVEMAKER\_REGULAR\_WAVE, WK\_WAVEMAKER\_IRREGULAR\_WAVE, CALCULATE\_Cm\_Sm, CALCULATE\_SPONGE if applied. 

\item {\bf INITIAL\_GAUSIAN:} sample to provide initial Gausian hump. Use flag `-DSAMPLE' in Makefile to include this subroutine.  Parameters  are defined in input.txt. The subroutine is called by INITIALIZATION. 

\item {\bf INITIAL\_HOTSTART:}  initialization when hotstart option is applied. It is called by MAIN.  It may call 
INITIAL\_UVZ, INITIAL\_SOLITARY\_WAVE,  INITIAL\_N\_WAVE, INITIAL\_RECTANGULAR,
INITIAL\_WAVE,
WK\_WAVEMAKER\_REGULAR\_WAVE, WK\_WAVEMAKER\_IRREGULAR\_WAVE, CALCULATE\_Cm\_Sm, CALCULATE\_SPONGE if applied. 

\item {\bf INITIAL\_N\_WAVE:}  sample to provide initial N wave solution. Use flag `-DSAMPLE' in Makefile to include this subroutine. Parameters  are defined in input.txt. The subroutine is called by INITIALIZATION. 

\item {\bf INITIAL\_SOLITARY\_WAVE:}   sample to provide initial solitary wave solution. Use flag `-DSAMPLE' in Makefile to include this subroutine. Parameters  are defined in input.txt. The subroutine is called by INITIALIZATION. It calls SUB\_SLTRY to get parameters.  

\item {\bf INITIAL\_RECTANGULAR:}   sample of given an initial rectangular hump. Use flag `-DSAMPLE' in Makefile to include this subroutine. Parameters  are defined in input.txt. The subroutine is called by INITIALIZATION. 

\item {\bf INITIAL\_UVZ:}  read initial $u, v$, and $ \eta$ data from files defined in input.txt. It is called by INITIALIZATION. It calls GetFile. 

\item {\bf MINMOD\_LIMITER:}  function of minmod limiter. It is used in  CONSTRUCT\_HO\_X,  and CONSTRUCT\_HO\_Y. 

\item {\bf MPI\_INITIAL:}  initialize MPI environment.

\item {\bf OneWayCoupling:} one-way nesting subroutine. The nesting data read from INITIALIZATION.

\item {\bf PHI\_EXCH:}  handle float type data  exchange between processors in the parallel mode. It is called by PHI\_COLL. Use flag `-DPARALLEL' to include this subroutine. 

\item {\bf PHI\_INT\_EXCH:}  handle integer type data  exchange between processors in the parallel mode. It is called by UPDATE\_MASK. Use flag `-DPARALLEL' to include this subroutine. 

\item {\bf PREDICTOR:}  predictor scheme. It was used for a comparison between Predictor-corrector scheme and Runge-Kutta. It is not suggested using it in the present program.  

\item {\bf PREVIEW:}  output subroutine. It is called by MAIN. It calls PutFile.  

\item {\bf PutFile:}   print out output in files. In the parallel mode, it
gathers data from all processors into processor 0 and prints out in the global dimension. Use flag '-DPARALLEL' in Makefile for the parallel mode. It is called by PREVIEW. 

\item {\bf PHI\_COLL:}  update data in ghost cells. It is called by EXCHANGE.  in the parallel mode, it calls PHI\_EXCH which is a major subroutine to handle data exchange between processors.  

\item {\bf READ\_INPUT:}  read input.txt.  It is called by MAIN. It calls GET\_STRING\_VAL, GET\_LOGICAL\_VAL, GET\_INTEGER\_VAL, 
GET\_Float\_VAL. Input data are written out in LOG.txt. 


\item {\bf READ\_HOTSTART\_DATA:}  read saved data for hot start and initialize other variables.

\item {\bf ScatterVariable:}  scatter variables from processor 0 to all processors in the parallel mode but with serial tridiagonal solver. Use flag `-DTRID\_NO\_PARALLEL' to include this subroutine. It is called by TRIDxyNoParallel. 

\item {\bf SOLITARY\_WAVE\_LEFT\_BOUNDARY:}  nudging boundary condition of solitary wave at left boundary. Use flag `-DSAMPLE' in Makefile to include this subroutine. Parameters  are defined in input.txt. 

\item {\bf SourceTerms:}  calculate all source terms including slope term and dispersion terms. It is called by MAIN. 

\item {\bf STATISTICS:}  calculate statistics of total mass volume, energy, maximum and minimum $\eta, u, v,$ Froude number etc. 

\item {\bf SPONGE\_DAMPING:}  use sponge layers to damp waves  Parameters  are defined in input.txt. 

\item {\bf SUB\_SLTRY:}  provide solitary wave solution of Nogu's equations. It is called by INITIAL\_SOLITARY\_WAVE. Use flag `-DSAMPLE' in Makefile to include this subroutine. 

\item {\bf TRI\_GE:}  tridiagonal solver. It allows diagonal variables not equal to unit. It is called by TRIG\_PERIODIC. 

\item {\bf TRIG:}  tridiagonal solver. It is called by GET\_Eta\_U\_V\_HU\_HV.  It calls TRI\_GE. 

\item {\bf TRIDxyNoParallel:}  no parallel tridiagonal solver but in the parallel mode. It is only for testing. Use flag `-DTRID\_NO\_PARALLEL' to include this subroutine. It calls GatherVariable, TRIG and ScatterVariable. 

\item {\bf TRIG\_PERIODIC:}  cyclic tridiagonal solver. It is called by GET\_Eta\_U\_V\_HU\_HV.  It calls TRI\_GE. 

\item {\bf UPDATE\_MASK:}  update mask according to wetting and drying. Also update mask and mask9 in ghost cells. 

\item {\bf VANLEER\_LIMITER:}   function of Vanleer limiter. It is used in DelxFun and  DelyFun. 

\item {\bf WAVE\_SPEED:}  calculate numerical wave speed needed by the TVD scheme. It is called by MAIN. 

\item {\bf WK\_WAVEMAKER\_IRREGULAR\_WAVE:}  calculate source function for Wei and Kirby's internal wave maker for irregular waves (TMA). Periodic boundary conditions are included.  Parameters  are defined in input.txt. The subroutine is called by INITIALIZATION. 

\item {\bf {\bf WK\_WAVEMAKER\_REGULAR\_WAVE:} } calculate source function for Wei and Kirby's internal wave maker for regular waves. Periodic boundary conditions are included but for certain wave angles (will ask during a run).  Parameters  are defined in input.txt. The subroutine is called by {\bf INITIALIZATION}. 

\item {\bf WRITE\_HOTSTART\_DATA:}  write out data used for hot start. 

\end{description}

\subsection{Permanent variables}

NOTE: The list below is not complete. Some more variables were added in different versions. 

\begin{description}

\item  Depth(): still water depth $h$ at element point
\item  DepthNode(): still water depth $h$  at node
...
\item  MASK9: mask to switch from Boussinesq equation to SWE, 1 - Boussinesq, 0 - SWE

\item  U():  depth-averaged $u$ or $ u$ at the reference level ($u_\alpha$) at element
\item  V():  depth-averaged $v$ or $ v$ at the reference level ($v_\alpha$) at element
\item  HU(): $(h+\eta)  u$ at element
\item  HV(): $(h+\eta)  v$ at element
\item  P(): $(h+\eta) (u + U_4)$ at x-interface
\item  Q(): $(h+\eta) (v + V_4)$ at y-interface
...
\item  U4(): x-component of $U_4$
\item  V4(): y-component of $V_4$
\item  U1p(): x-component of $U_1^\prime$
\item  V1p(): y-component of $V_1^\prime$
...
\item  PL(): $(h+\eta) (u + U_4)$, Left value at x-interface
\item  PR(): $(h+\eta) (u + U_4)$, Right value at x-interface
\item  QL(): $(h+\eta) (v + V_4)$, Left value at y-interface
\item  QR(): $(h+\eta) (v + V_4)$, Right value at y-interface

\item  FxL = HUxL*UxL + 1/2*g*(EtaRxL$^2$ + 2*EtaRxL*Depthx)
\item  FxR = HUxR*UxR + 1/2*g*(EtaRxR$^2$ + 2*EtaRxR*Depthx)
\item  FyL = HyL*UyL*VyL
\item  FyR = HyR*UyR*VyR

\item  GxL = HxL*UxL*VxL
\item  GxR = HxR*UxR*VxR
\item  GyL = HVyL*VyL + 1/2*g*(EtaRyL$^2$ + 2*EtaRyL*Depthy)
\item  GyR = HVyR*VyR + 1/2*g*(EtaRyR$^2$ + 2*EtaRyR*Depthy) 
